...
impl Plugin for HeavyPlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(HeavyTimer(Timer::new(HEAVYNESS_DURATION, TimerMode::Once)))
            .add_systems(Update, (tick_timer, update_mass, change_color).chain());
    }
}

#[derive(Resource)]
struct HeavyTimer(Timer);
...
fn update_mass(
    ...
    *additional_mass = if k_in.pressed(HEAVY_CODE) {
        AdditionalMassProperties::Mass(timer.0.remaining().as_secs_f32() * HEAVINESS_FACTOR)
    } else {
...
fn change_color(
    ...
    if let Some(material) = assets.get_mut(&ball.material_handle) {
        material.color = if k_in.pressed(HEAVY_CODE) {
            apply_saturation_ratio(BALL_COLOR, timer.0.percent())
        } else {
            BALL_COLOR
        }
    }
...
fn backwards_tick(timer: &mut Timer, delta: Duration) {
